...
app.get('/api/portfolios', async (_req, res) => {
  try {
    ...
    const files = fs.readdirSync(configPath).filter(f => f.startsWith('portfolio-') && f.endsWith('.json')).sort();
    const portfolios = files.map(filename => {
      const content = fs.readFileSync(path.join(configPath, filename), 'utf8');
      return { id: filename.replace('.json', ''), filename, portfolio: JSON.parse(content) };
    });
...
app.get('/api/scenarios', async (_req, res) => {
  try {
    ...
    if (!fs.existsSync(scenariosPath)) return res.json([]);
    const files = fs.readdirSync(scenariosPath).filter(f => f.endsWith('.json')).sort();
...
app.get('/api/thresholds/:jurisdiction', (req, res) => {
  ...
  return res.json(presets[jurisdiction] || presets['custom']);
});

app.get('/api/environments', (_req, res) => {
  const envList = Object.entries(ENVIRONMENTS).map(([name, config]) => ({
    name, riskServiceUrl: config.riskServiceBase, actusServerUrl: config.actusServerBase,
  }));
...
app.get('/api/stimulations', async (_req, res) => {
  try {
    ...
    if (!fs.existsSync(stimulationPath)) return res.json([]);
    const results: any[] = [];
    ...
    for (const subdir of subdirs) {
      ...
      for (const filename of files) {
        try {
          ...
          results.push({ id: `${subdir.name}/${filename.replace('.json', '')}`, category: subdir.name, filename, name: collection.info?.name || filename, description: collection.info?.description || '', stepsCount: collection.item?.length || 0 });
        } catch { }
      }
    }
...
app.post('/api/stimulation/run', async (req, res) => {
  try {
    ...
    if (!stimulationId && !rawCollection) return res.status(400).json({ success: false, error: 'Missing required field: provide either stimulationId or collectionJson' });
    let envConfig: EnvironmentConfig; let envName: string;
    if (customUrls && customUrls.riskServiceBase && customUrls.actusServerBase) { envConfig = { riskServiceBase: customUrls.riskServiceBase, actusServerBase: customUrls.actusServerBase }; envName = 'custom'; }
    else { envName = environment || 'localhost'; envConfig = ENVIRONMENTS[envName]; if (!envConfig) return res.status(400).json({ success: false, error: `Unknown environment: "${envName}". Available: ${Object.keys(ENVIRONMENTS).join(', ')}` }); }
    let collectionJson: any; let sourceLabel: string;
    if (rawCollection && rawCollection.item && Array.isArray(rawCollection.item)) { collectionJson = rawCollection; sourceLabel = rawCollection.info?.name || 'uploaded-collection'; console.error(`\n🎯 Stimulation Request (uploaded): ${sourceLabel}`); }
      ...
      if (!fs.existsSync(filePath)) return res.status(404).json({ success: false, error: `Stimulation file not found: ${stimulationId}.json` });
      collectionJson = JSON.parse(fs.readFileSync(filePath, 'utf8')); sourceLabel = stimulationId; console.error(`\n🎯 Stimulation Request: ${sourceLabel}`);
    } else return res.status(400).json({ success: false, error: 'Invalid collectionJson' });
    console.error(`   Environment: ${envName}`); console.error(`   Risk Service: ${envConfig.riskServiceBase}`); console.error(`   ACTUS Server: ${envConfig.actusServerBase}`);
    const result = await runStimulation(collectionJson, envConfig, envName);
    return res.json(result);
  } catch (error: any) { console.error('Stimulation error:', error); return res.status(500).json({ success: false, error: error.message, timestamp: new Date().toISOString() }); }
});

app.get('/api/health/risk-service', async (req, res) => {
  const envName = (req.query.environment as string) || 'localhost';
  const envConfig = ENVIRONMENTS[envName];
  if (!envConfig) return res.status(400).json({ error: `Unknown environment: ${envName}` });
  const checks: Record<string, any> = { environment: envName, timestamp: new Date().toISOString() };
  try { const axios = await import('axios'); const response = await axios.default.get(`${envConfig.riskServiceBase}/findAllScenarios`, { timeout: 5000 }); checks.riskService = { url: envConfig.riskServiceBase, connected: true, status: response.status }; } catch (error: any) { checks.riskService = { url: envConfig.riskServiceBase, connected: false, error: error.message }; }
  try { const axios = await import('axios'); const testPayload = { contracts: [{ contractType: 'PAM', contractID: 'health_check', contractRole: 'RPA', contractDealDate: '2024-01-01T00:00:00', initialExchangeDate: '2024-01-01T00:00:00', statusDate: '2024-01-01T00:00:00', notionalPrincipal: 1000, maturityDate: '2024-01-02T00:00:00', nominalInterestRate: 0.0, currency: 'USD', dayCountConvention: 'A365' }], riskFactors: [] }; const response = await axios.default.post(`${envConfig.actusServerBase}/eventsBatch`, testPayload, { headers: { 'Content-Type': 'application/json' }, timeout: 5000 }); checks.actusServer = { url: envConfig.actusServerBase, connected: response.status === 200 && !!response.data, status: response.status }; } catch (error: any) { checks.actusServer = { url: envConfig.actusServerBase, connected: false, error: error.message }; }
  return res.json({ status: checks.riskService?.connected && checks.actusServer?.connected ? 'healthy' : 'degraded', ...checks });
});
...
app.listen(PORT, () => {
  ...
  console.error('  ─────────────── Claude.ai GET Endpoints (NEW) ───────────────');
  console.error(`   Verify:        GET http://localhost:${PORT}/api/claude/verify/:filename`);
